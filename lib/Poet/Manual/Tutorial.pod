...
Run this to create the initial environment:
    ...
    blog/app.psgi
    ...
    Now run 'blog/bin/run' to start your server.
...
For any website it's a good idea to have a well-defined I<model> through which
you retrieve and change data. Poet and Mason don't have much to say about how
you do this, so we'll make some minimal reasonable choices here and move on.
...
More generally C<$env> is one of several special Poet variables that can be
...
See L<Poet::Import|Poet::Import> for the full list of Poet variables.
...
component|Mason::Manual::Components/COMPONENT FILES>. This component decides
the overall page layout, and then may call other components to fill in the
details.

Here's our first component to serve the home page, C<comps/index.mc>:
...
Imports the specified variables - in this case C<$conf> and C<$env> - into the
script namespace. The available variables are the same as discussed in L</POET
VARIABLES>.

=back

Poet initialization has to happen exactly once per process, before any Poet
features are used. In fact, take a look at C<bin/run> -- which was generated
