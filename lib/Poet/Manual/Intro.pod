...
Imports the specified I<quick vars> - in this case C<$conf> and C<$env> - into
...
More information about this C<$env> variable L<below|QUICK VARS AND UTILITIES>.
...
More information about this C<$conf> variable L<below|QUICK VARS AND
UTILITIES>.
...
C<poet new> generates C<run.pl> and C<app.psgi> files for you in the C<bin>
subdirectory.

C<run.pl> is a wrapper around L<plackup>, which starts your server. It has a
few sensible defaults, such as setting up autorestart in development mode and
using an access log in live mode. It will also take options from the
configuration entry 'plackup'. So if your C<local.cfg> contains this:

    plackup:
       port: 5000

then --port 5000 will be passed to plackup.
...
C<app.psgi> defines your PSGI app. It's the place to add L<Plack
...
Poet makes it easy to import certain variables (known as "quick vars") and sets
of utilities into any module or script in your environment. You've seen two
